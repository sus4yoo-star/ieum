...
export const EMPTY_MEMORY: MannaMemory = {
  recurring_struggles: [],
  recent_emotions: [],
  prayer_topics: [],
  faith_state: "",
...
export function normalizeMemory(raw: any): MannaMemory {
...
export function memoryIsEmpty(memory?: MannaMemory | null): boolean {
...
export function renderMemoryForPrompt(memory?: MannaMemory | null): string {
  ...
  if (m.faith_state) {
    lines.push(`Belief/worldview notes (only if the user raised it): ${m.faith_state}`);
  }
  if (m.relationship_themes.length) {
    lines.push(`Relationship themes: ${m.relationship_themes.join("; ")}`);
  }
  if (m.prayer_topics.length) {
    lines.push(`Things they asked to keep in mind: ${m.prayer_topics.join("; ")}`);
  }
...
export async function loadUserMemory(supabase: any, userId: string): Promise<MannaMemory> {
  try {
    ...
    // The table may not exist yet. 셀라 must still work.
    return { ...EMPTY_MEMORY };
  }
}
...
export async function updateUserMemory({
  ...
  try {
    ...
    const prompt = `You update 셀라's long-term companion conversation memory.
...
- Add only durable, genuinely useful context that helps future conversations feel remembered.
- Leave "faith_state" and "prayer_topics" EMPTY unless the user themselves explicitly brought up their faith, religion, or beliefs. Never infer or impose them.
